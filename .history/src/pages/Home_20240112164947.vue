...
const favorite = (tool: ToolsType, event) => {
    event.stopPropagation(); // 阻止事件冒泡到外部元素
    ...
    <div style="padding: 8px">
        <a-row :gutter="16">
            <a-col :span="6" v-for="item in tools" style="margin-top: 12px;">
                <a-card :title="item.name" @click="console.log('ggggggggggg')">
                    <template #extra>
                        <a-tooltip placement="bottom">
                            ...
                            <HeartOutlined @click.native="favorite(item)" :style="{ color: item.isLike ? 'hotpink' : '' }" />
                        </a-tooltip>
                    </template>
