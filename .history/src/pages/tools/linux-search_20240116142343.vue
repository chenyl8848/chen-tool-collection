...
const modules = import.meta.glob('/node_modules/linux-command/command/*.md')
import { ref } from 'vue';

import fs from 'fs'

// import alias from "linux-command/command/7z.md";
// console.log(alias)
...
const getCommandInfo = (path: string) => {
    info.value = modules[`/node_modules/linux-command/command${path}.md`]
    let data:Buffer = fs.readFileSync(`/node_modules/linux-command/command${path}.md`)
    console.log(path, "pathhhhhhhhh", data.toString())
}
