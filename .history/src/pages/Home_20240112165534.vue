...
// 收藏
const favorite = (tool: ToolsType, event) => {
    event.stopPropagation();
    console.log(tool, "tttttttttt")
}

const clickTool = (tool: ToolsType) => {
    router.push(tool.path)
    console.log(tool, "ttt123132")
}
</script>
    ...
    <div style="padding: 8px">
        <a-row :gutter="16">
            <a-col :span="6" v-for="item in tools" style="margin-top: 12px;">
                <a-card :title="item.name" @click="clickTool(item)">
                    <template #extra>
                        <a-tooltip placement="bottom">
                            ...
                            <HeartOutlined @click.native="favorite(item, $event)"
                                :style="{ color: item.isLike ? 'hotpink' : '' }" />
                        </a-tooltip>
                    </template>
